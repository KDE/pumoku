...
SudokuBoard::Difficulty SudokuBoard::getDifficulty(){
	// if (getGuessCount() > 10) return SudokuBoard::DIABOLICAL;
	if (getGuessCount() > 0) return SudokuBoard::EXPERT; // GRADES + MORE STRATEGIES
	if (getBoxLineReductionCount() > 0) return SudokuBoard::INTERMEDIATE; // devious
	if (getPointingPairTripleCount() > 0) return SudokuBoard::INTERMEDIATE; // annoying
	if (getHiddenPairCount() > 0) return SudokuBoard::INTERMEDIATE; // difficult
	if (getNakedPairCount() > 0) return SudokuBoard::INTERMEDIATE; // intricate
	if (getHiddenSingleCount() > 0) return SudokuBoard::EASY; // moderate
	if (getSingleCount() > 0) return SudokuBoard::SIMPLE; // easy - GRADES
	return SudokuBoard::UNKNOWN;
}

/**
	* Get the difficulty rating. FIXME see above
	*
